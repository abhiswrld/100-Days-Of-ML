...
* **Statistics:** `scipy`
* **Machine Learning & Preprocessing:** `scikit-learn` (Pipelines, Transformers, Encoders, Imputers, GridSearchCV, Linear & Tree Models)
...
* **Day 41:** Covered the theory of outliers in ML. Learned detection techniques based on data distribution (Z-Score for normal, IQR for skewed, Percentile-based) and various treatment methods like Trimming, Capping (Winsorization), and Discretization.
* **Day 42:** Implemented Z-Score outlier detection for normally distributed data. Handled outliers using trimming (dropping rows outside 3 standard deviations) and capping (restricting extreme values using `np.where`).
* **Day 43:** Implemented IQR (Interquartile Range) outlier detection for skewed distributions. Visualized outliers using Seaborn boxplots and treated them using both trimming and capping (Winsorization) techniques.
